...
\begin{table}[!htb]
	...
	\scriptsize{
		\begin{tabular}{llc}
			...
			%|rule
			\midrule
			responded existence  &  $\lfuture A \rightarrow \lfuture B$ & $A$\\
			co-existence & $\lfuture A \leftrightarrow \lfuture B$ & $A$, $B$ \\
			\midrule
...
The standard semantics of \Declare\ is extended by requiring additional conditions on data, i.e., the \emph{activation condition}, the \emph{correlation condition}, and a \emph{time condition.} As an example, we consider
a Response(ship order, send invoice) constraint, with
...
Recognizing its potential as well as its limitations, we extend this approach as follows: (1) we generalized the pattern recognition mechanisms in order to handle more flexible inputs, (2) we cover eight additional constraint templates, and (3) we add support for augmentation with data and time conditions.
