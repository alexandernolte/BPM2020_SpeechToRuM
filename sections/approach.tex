...
In this section, we outline our conceptual contributions with respect to the first two functions, i.e., for the transformation from natural language text into declarative constraints (Sect.~\ref{sec:texttodeclareconstraints}) and for the augmentation of constraints with multi-perspective conditions (Sect.~\ref{sec:multiperspectiveaugmentation}).
We cover the implementation of the approach and the user interaction it supports in Sect.~\ref{sec:implementation}.
...
%	Since the user indicates which of these three will be added prior to recording its description, our approach is automatically aware of the type.
	...
	\mypar{Activation conditions} Activation conditions denote specific requirements that must be met in order for a constraint to be applicable, e.g., stating that \textit{Response(ship order, send invoice)} should only apply when the amount associated with \emph{ship order} activity is above 500.
...
\mypar{Time conditions} A time condition bounds the time between the occurrence of the activation and target of a constraint, e.g., stating that a response should occur within 5 days after the activation.
...
%Furthermore, when specifying a range, the time unit can be different for the lower and the upper bound, as seen for the third pattern in the table.
