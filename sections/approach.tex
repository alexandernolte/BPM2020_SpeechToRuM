...
\todo{Han: this section probably should have a different name}
...
In this section, we outline our conceptual contributions with respect to the first two functions, i.e., for the transformation from natural language text into declarative constraints (\autoref{sec:texttodeclareconstraints}) and into augmentation of constraints with multi-perspective constraints (\autoref{sec:multiperspectiveaugmentation}).
...
\subsection{Constraint generation}
	...
	In this component, our approach aims to turn a constraint description, recorded using speech speech, into one or more declarative constraints. 
...
\subsection{Multi-Perspective Augmentation}
	...
	Our approach supports the augmentation of declarative constraints with conditions on the data and temporal perspectives, allowing \Declare\ constraints to be turned into \MPDeclare\ ones. Our approach allows users to express three types of conditions through speech input, i.e., 
...
Our approach handles two patterns here, either allowing a user to express that an attribute should be equal for both activities, e.g., ``\emph{The color is the same}'' or that they not equal, e.g., ``\emph{The applicant is different}''. \todo{Han: should we justify why we have such simple patterns? Is this simply expressivene enough?}


\mypar{Time conditions}
