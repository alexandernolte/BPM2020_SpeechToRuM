...
\subsection{Multi-Perspective Augmentation}
	...
	\mypar{Activation conditions} Activation conditions define constraints on attribute values associated with the \emph{activation} activity of a constraint. They denote specific requirements that must be met in order for a constraint to be applicable, e.g., stating that Response(receive loan application, check loan application) should only apply when the amount associated with the application is above 50 000EUR.
	
	Our approach is able to handle input that follows the patterns shown in \autoref{tab:activiationpatterns}. 
	
	\begin{table}[!h]
		\caption{Condition patterns supported by Speech2RuM}
		\label{tab:activiationpatterns}
		\begin{tabular}{lll}
			\toprule
			\textbf{Type} & \textbf{Pattern} & \textbf{Semantics} \\ 
			\midrule
