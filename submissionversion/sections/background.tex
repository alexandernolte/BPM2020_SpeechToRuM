...
\begin{table}[!htb]
	...
	%		alternate precedence & $\lglobally(B \rightarrow \lyesterday(\neg B \lsince A ))$ & $B$\\
			...
			% |rule
			not coexistence  &  $\lfuture A \rightarrow \neg \lfuture B$ & $A$, $B$\\
...
constraint $\lglobally(a \rightarrow \lfuture b)$ and $b$ is a target, because the execution of $a$ forces $b$ to be executed, eventually. In \tablename~\ref{tbl:timed-ltl}, for each template the corresponding activations are specified.
